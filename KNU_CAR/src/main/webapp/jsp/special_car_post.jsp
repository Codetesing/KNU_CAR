    ...
    <%@ page language="java"
     ...
     public void SPECIAL_CAR_INSERT(Connection conn, String id, String cid) throws SQLException {
	    	PreparedStatement pstmt;
 			...
 			query = "SELECT COUNT(*) FROM SPECIAL_CAR";
			...
			if(rs.next()) {
				num = rs.getInt(1) + 1;
				int special_id = num;
				String special_car_id = car_id;
				String special_user_id = user_id;
				
				
				//쿼리생성 차량 기본정보  
				query = "INSERT INTO SPECIAL_CAR VALUES ('"
						+ special_id + "','"
						+ special_car_id + "','"
						+ special_user_id + "','"
						+ mileage + "','"
				...
				if(option != null && option.length > 0) {
					query = "SELECT COUNT(*) FROM SPECIAL_CAR_OPTION";
					pstmt = conn.prepareStatement(query);
					rs = pstmt.executeQuery();
					...
					if(rs.next()) {
						...
						for(int i = 0; i < option.length; i++) {
							String opt = option[i];
							query = "INSERT INTO SPECIAL_CAR_OPTION VALUES ('" + special_id + "','" + special_car_id + "','"
									+ special_user_id + "','" + opt + "')";
				...
				if(agreements != null && agreements.length > 0) {
					for(int i = 0; i < agreements.length; i++) {
							query = "SELECT COUNT(*) FROM SPECIAL_CAR_ACCIDENT_HISTORY";
							pstmt = conn.prepareStatement(query);
							rs = pstmt.executeQuery();
							...
							query = "INSERT INTO SPECIAL_CAR_ACCIDENT_HISTORY VALUES ("+ 
							accident_id + ", '"+ special_id + "','" + special_car_id + "','" + special_user_id + "'," + 
							insurance_accident + ", " + waterlogged + ", " + stolen + ", " + total_loss + ", " + change_use + ")";
...
<body onload="noBack();" onpageshow="if(event.persisted) noBack();" onunload="">
	...
	<%
		...
		lift_axle = request.getParameter("lift_axle");
